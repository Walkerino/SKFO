...
EOT;
	...
	protected function modifyInputfield(HookEvent $event) {
		...
		$button_text = $this->_('Вставить');
		if($is_image_inputfield) {
			$placeholder_text = $this->_('Вставь ссылки на картинку здесь, одна ссылка - одна линия');
		} else {
			$placeholder_text = $this->_('Вставь ссылки на файл здесь, одна ссылка - одна линия');
		}
